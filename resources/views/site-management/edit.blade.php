...
<div class="flex flex-col gap-4 bg-white w-full rounded-2xl p-4">
    <div class="flex justify-between items-center">
        <h1 class="text-2xl">Edit Site</h1>
    ...
    <form action="{{ route('site-management.update', $site->id) }}" method="POST" class="space-y-4">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div class="flex flex-col gap-2">
                ...
                <input type="tel" name="site_phone" id="site_phone" 
                    ...
                    value="{{ old('site_phone', $site->site_phone) }}">
                @error('site_phone')
                    <p class="text-red-500 text-xs">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="flex flex-col gap-2">
                ...
                <input type="email" name="site_email" id="site_email" 
                    ...
                    value="{{ old('site_email', $site->site_email) }}">
                @error('site_email')
                    <p class="text-red-500 text-xs">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="flex flex-col gap-2 md:col-span-2">
                ...
                <textarea name="site_address" id="site_address" rows="3" 
                    ...
                    >{{ old('site_address', $site->site_address) }}</textarea>
                @error('site_address')
                    <p class="text-red-500 text-xs">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="flex flex-col gap-2">
                ...
                <input type="text" name="site_longitude" id="site_longitude" 
                    ...
                    value="{{ old('site_longitude', $site->site_longitude) }}">
            ...
            <div class="flex flex-col gap-2">
                ...
                <input type="text" name="site_latitude" id="site_latitude" 
                    ...
                    value="{{ old('site_latitude', $site->site_latitude) }}">
                @error('site_latitude')
                    <p class="text-red-500 text-xs">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="flex flex-col gap-2">
                ...
                <select name="site_visibility" id="site_visibility" 
                    ...
                    <option value="all" {{ old('site_visibility', $site->site_visibility) == 'all' ? 'selected' : '' }}>All</option>
                    <option value="private" {{ old('site_visibility', $site->site_visibility) == 'private' ? 'selected' : '' }}>Private</option>
                </select>
            ...
            <div class="flex flex-col gap-2">
                ...
                <select name="site_status" id="site_status" 
                    ...
                    <option value="1" {{ old('site_status', $site->site_status) == 1 ? 'selected' : '' }}>Active</option>
                    <option value="0" {{ old('site_status', $site->site_status) == 0 ? 'selected' : '' }}>Inactive</option>
                </select>
                @error('site_status')
                    <p class="text-red-500 text-xs">{{ $message }}</p>
                @enderror
...
<script>
    $(document).ready(function() {
        // Get initial coordinates from the site data or use default Jakarta coordinates
        var initialLat = {{ old('site_latitude', $site->site_latitude) ?? -6.200000 }};
        var initialLng = {{ old('site_longitude', $site->site_longitude) ?? 106.816666 }};
        ...
        $('#site_latitude, #site_longitude').on('change', function() {
            var lat = parseFloat($('#site_latitude').val()) || initialLat;
            var lng = parseFloat($('#site_longitude').val()) || initialLng;
