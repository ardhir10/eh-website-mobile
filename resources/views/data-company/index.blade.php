...
<div class="flex flex-col gap-4 bg-white w-full rounded-2xl p-4">
    <div class="flex justify-between items-center">
        ...
        <a href="{{ route('data-company.create') }}" class="px-4 py-2 bg-magenta-2 text-white text-sm rounded-full hover:bg-magenta-3 flex items-center gap-2">
        ...
        <table id="companyTable" class="w-full text-sm text-left text-gray-500">
            <thead class="text-xs uppercase text-[#357AA6]">
                <tr>
                    ...
                    <th scope="col" class="!px-2 !py-4">Company Logo</th>
            ...
            <tbody>
                @foreach($companies as $company)
                    <tr class="bg-white border-b">
                        ...
                        <td class="px-6 !py-3">{{ $company->company_name }}</td>
                        <td class="px-6 !py-3">
                            <div class="max-w-xs truncate" title="{{ $company->company_address }}">
                                {{ $company->company_address }}
                            </div>
                        </td>
                        <td class="px-6 !py-3 space-x-2">
                            <a href="{{ route('data-company.show', $company) }}" 
                               class="text-green-500 hover:text-green-700">
                                <i class="fas fa-eye"></i> 
                            </a>
                            <a href="{{ route('data-company.edit', $company) }}" 
